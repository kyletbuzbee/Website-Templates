...
function validateField(field) {
    ...
    switch (fieldName) {
        ...
        case 'message':
            if (!value) {
                isValid = false;
                errorMessage = 'Please describe your inquiry';
...
function handleFormSubmit(e) {
    ...
    if (validateForm()) {
        ...
        setTimeout(() => {
            ...
            showNotification('Thank you! Your inquiry has been submitted. Our customer support team will respond within 24 hours.', 'success');
...
function initializeEcommerceFeatures() {
    // Order number field toggle based on inquiry type
    const inquirySelect = document.querySelector('#inquiryType');
    const orderNumberGroup = document.querySelector('#orderNumber').closest('.form-group');

    if (inquirySelect) {
        inquirySelect.addEventListener('change', function() {
            const orderNumberInput = document.querySelector('#orderNumber');

            if (this.value === 'order' || this.value === 'returns') {
                orderNumberGroup.style.display = 'block';
                orderNumberInput.required = true;
                orderNumberInput.setAttribute('aria-required', 'true');
            } else {
                orderNumberGroup.style.display = 'none';
                orderNumberInput.required = false;
                orderNumberInput.removeAttribute('aria-required');
                // Clear any existing validation errors
                const errorMsg = orderNumberInput.parentNode.querySelector('.error-message');
                if (errorMsg) {
                    errorMsg.remove();
                }
        ...
        // Initially hide order number field
        orderNumberGroup.style.display = 'none';
    }
    ...
    if (newsletterCheckbox) {
        newsletterCheckbox.addEventListener('change', function() {
            if (this.checked) {
                // Could trigger additional newsletter preferences or confirmation
                console.log('Newsletter signup enabled');
            }
        });
    }
}
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Initialize e-commerce specific features
...
// E-commerce specific accessibility features
document.addEventListener('DOMContentLoaded', function() {
