...
class ABTestingManager {
   ...
   */
  setSafeStorage(key: string, value: any): boolean {
    try {
      if (typeof Storage === 'undefined') {
        console.warn('AB Testing: localStorage not available');
        return false;
    ...
    if (stored && typeof stored === 'object') {
      Object.entries(stored).forEach(([expId, expData]) => {
        if (this.isValidExperiment(expData)) {
          this.experiments.set(expId, expData as Experiment);
        }
      });
    }
   ...
   */
  isValidExperiment(exp: any): boolean {
    return (
      exp &&
      typeof exp === 'object' &&
      exp.id &&
  ...
  getVariant(experimentId: string): string | null {
    ...
    if (experiment.status !== 'active') {
      return experiment.variants[0] || 'A'; // Default to first variant
    }
   ...
   */
  selectVariant(experiment: Experiment, userId: string): string {
   ...
   */
  getUserAssignment(experimentId: string): any {
   ...
   */
  setUserAssignment(experimentId: string, variant: string): void {
    const assignments = this.getSafeStorage(`${this.storageKey}_assignments`) || {};
   ...
   */
  trackConversion(experimentId: string, goalName: string, metadata: any = {}): void {
    const experiment = this.experiments.get(experimentId);
    if (!experiment) {
      return;
    }
   ...
   */
  endExperiment(experimentId: string, winnerVariant: string | null = null): void {
    const experiment = this.experiments.get(experimentId);
    if (!experiment) {
      return;
    }
...
(window as any).ABTestingManager = ABTestingManager;
